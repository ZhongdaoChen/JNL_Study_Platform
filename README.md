...
4. **中文读发音练习**：中文读为语音练习模式，评分全自动：不显示四个手动评分按钮，A/S/D/F 快捷键同时停用。首次点击麦克风只会触发浏览器麦克风权限，请求成功后当前中文读会话会复用同一条音频流；录音不足 250ms 会在浏览器端直接要求重试，合格录音会降采样为 16 kHz 单声道 PCM WAV 后提交，也会在说话后静音约 800ms 或最晚 6 秒时自动停止。评估由支持原始音频输入的模型一次完成转写与带置信度的判断：只有高置信度的明确结论才评分，听不清或低置信度结果不会评分。第一遍读对自动判为「熟练」，播放约 2 秒加大版烟花庆祝动画后自动进入下一个词；第一遍读错自动判为「略陌生」，提示「读错了」后同样自动进入下一个词，不再产生当天补做；若孩子在此期间点了「听正确读音」，自动进位会让行到播放（含辅助词）结束，再等一个反馈窗口后才切换。返回已评过的词重试只更新提示，不会再次改分，仍可「听正确读音」（目标字 + 辅助词，点击时按需合成）。单字的任一常见现代普通话读音都可通过，并会自动准备 3 个包含目标字的辅助词；词组 / 句子只播放完整标准读音。中文读关闭倒计时，其他复习模式保持原有规则。
