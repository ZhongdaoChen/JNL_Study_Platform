...
前端 (React + TS + Vite, 纯静态)
  ...
  │        Workspace 标签: 录入/复习/总览/统计 (+ 管理员，仅管理员邮箱可见)
  ...
  └─ 逻辑层 src/lib/
       ...
       wordService.ts 业务: addLearning(lang) / getDueReviews(lang) / submitReview
...
5 张表，全部启用 RLS，按 `auth.uid()` 隔离（每个家长只看自己数据）：
- `children`(owner=auth.uid(), name, created_at)
- `sentences`(child_id, text, created_at)
- `words`(child_id, text, **lang('en'|'zh', 默认 en)**, sentence_ids[], first_learned_at, **example_sentence**, interval, ef, repetitions, due_date, last_grade, last_reviewed_at, unique(child_id,text))
...
> ⚠️ 部署/升级后必须在 Supabase SQL Editor **重跑** `supabase/schema.sql`（幂等安全，无 drop/delete/truncate，不丢数据）。否则线上中文录入/反馈/管理员页会报错。
...
- 中英文共用同一套算法，仅按 `lang` 过滤（en/zh 字符集不重叠，无冲突）
...
- 关键提交(新→旧):
  ...
  - `51387ac` 版本更新框仅在录入页显示
  - `af3f57f`/`38b3b82` 录入页底部版本更新记录
  - `e5b6ee9` 中文学习(按单字拆分, 复习/总览分中英标签)
  - `fb14afe` AI 例句生成(通义千问)
  - `cc82feb` PROJECT_STATUS.md
...
### 待用户在后台操作（线上生效前必须做）
- [ ] ⚠️ Supabase SQL Editor **重跑 schema.sql**(幂等不丢数据) → 补 lang/example_sentence 列、建 feedback 表、建 admin_* 函数
- [ ] ⚠️ Vercel 后台新增环境变量 `QWEN_API_KEY`(无 VITE_ 前缀, 否则 AI 例句报错)
- [ ] ⚠️ 建议轮换泄露的 Qwen Key(曾明文发送): sk-c97b01577be14f2490c68e22158dd348
...
- `src/components/Workspace.tsx` 主体, 标签栏(含管理员条件渲染)、中英子标签、检测管理员邮箱
- `src/components/LearnInput.tsx` 录入(英文/中文切换)
- `src/components/ReviewSession.tsx` 复习(lang 过滤, AI 例句, 进度持久化, effect 依赖 [childId, lang])
- `src/components/WordList.tsx` 总览(lang 过滤)
...
- `src/lib/sm2.ts` 算法参数
- `supabase/schema.sql` 数据库结构+RLS+管理员函数(需重跑生效)
